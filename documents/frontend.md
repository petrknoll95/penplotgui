...
The frontend is a Vite + React 18 + TypeScript application in [`frontend/`](../frontend). It is an operational control surface for a pen plotter: upload SVG, preview toolpaths, position and scale artwork, start/test plots, jog the machine, and adjust settings.
...
| Component primitives | Local wrappers over `@base-ui/react` primitives |
...
[`App.tsx`](../frontend/src/App.tsx) renders a fixed grid layout:

- App header: spans the full viewport width and contains live X/Y/Z readouts, a status-aware connect/disconnect button with tooltip disclosure, `Test`, `Start Plot`, and a right-aligned settings icon button.
- Left sidebar: `320px`, fixed width, with upload and drawing-preparation panels.
- Main work area: `1fr`, dark gridded background, scrollable preview surface.
- Right sidebar: `320px`, fixed width, with the `Manual Control` panel.
- Timeline scrubber: sticky at the bottom of the main work area when SVG paths exist and the plotter is not actively plotting.
...
| `isLoading` | Upload/processing state while a file is being accepted. |
| `isPreviewUpdating` | Preview-processing state used to pulse the canvas during expensive repositioning. |
...
| `isSettingsOpen` | Controls the settings dialog opened from the header. |
...
| [`PositionControls`](../frontend/src/components/PositionControls.tsx) | Artboard preset dropdown, orientation, custom artboard size, scale mode, two-row horizontal/vertical alignment controls, margin, and custom offset. |
| [`OptimizationControls`](../frontend/src/components/OptimizationControls.tsx) | Path order method selector. |
| [`JogControls`](../frontend/src/components/JogControls.tsx) | Jog distance, X/Y/Z jogging, pen up/down, stop/reset, home all axes, and open set-home modal. |
...
| [`Settings`](../frontend/src/components/Settings.tsx) | Dialog content for soft-limit bed width/height, rapid speed slider, draw speed slider, and easing toggle. |
...
| [`Dialog`](../frontend/src/components/ui/dialog.tsx) | Base UI dialog wrapper used by Settings and Set Home; uses shared modal scale/fade motion from `index.css`. |
| [`Tooltip`](../frontend/src/components/ui/tooltip.tsx) | Base UI tooltip wrapper used for header action disclosure. |
...
| Connect/disconnect from header | `connect()`, `disconnect()` | `POST /api/plotter/connect`, `POST /api/plotter/disconnect` |
...
| Stop/reset from Manual Control | `stopPlot()`, `reset()` | Plotter control endpoints |
| Home/jog/pen from Manual Control | `home()`, `jog()`, `penUp()`, `penDown()` | Plotter control endpoints |
...
| Settings dialog | `setBedSize()`, `setSpeedSettings()` | Plotter settings endpoints |
| API-only settings helper | `setStepsPerMm()` | Plotter settings endpoint not currently exposed in the visible Settings dialog |
...
When upload or reposition processing is active, `SvgPreview` receives `isUpdating` and applies a simple `animate-pulse`/opacity state to the rendered canvas so complex SVG processing has visible feedback.
...
- Typography defaults to Inter with `text-sm`; monospace is reserved for numeric readouts and code-like values.
...
- Sidebar panels are collapsible Base UI primitives and can receive a drag handle from `DraggablePanel`.
- Dialogs use the shared Transitions.dev-style `.t-modal` motion in `index.css`: 250 ms open, 150 ms close, scale from `0.96`, opacity fade, and a reduced-motion fallback.
- Header tooltips use the local Base UI tooltip wrapper.

## Frontend Persistence

Only panel order is stored in browser persistence:

- Keys: `sidebar-prepare-panel-order` and `sidebar-machine-panel-order`.
- Legacy key still read for migration: `sidebar-panel-order`.
- Default prepare order: `position`, `optimization`.
- Default machine order: `jog`.
...
- `pausePlot()` and `resumePlot()` exist in the frontend API helper, but the current visible controls do not expose pause/resume buttons.
